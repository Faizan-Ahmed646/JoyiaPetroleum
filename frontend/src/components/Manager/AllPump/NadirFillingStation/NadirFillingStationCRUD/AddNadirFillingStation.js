...
const AddNadirFillingStation = () => {
    ...
    const saveData = async (e) => {
        ...
        e.preventDefault();
        let pumpData = await fetch("https://backend.joyiapetroleum.com/joiya/nadirFillingStations/data/", {
            method: 'POST',
            body: JSON.stringify({
    ...
    const userData = async () => {
        let res = await fetch("https://backend.joyiapetroleum.com/joiya/nadirFillingStations/data")
        res = await res.json();
    ...
    return (
        <>
            <div className="form-container card border-0 shadow rounded-3 my-5 p-5">
                <h1 className="mb-5">Nadir Filling Station</h1>
                <hr />
                <form>
...
export default AddNadirFillingStation
