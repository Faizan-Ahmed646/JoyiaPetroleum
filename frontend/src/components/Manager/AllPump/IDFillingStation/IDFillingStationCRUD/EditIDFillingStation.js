...
const EditIDFillingStation = () => {
    ...
    const getUserDetail = async () => {
        let res = await fetch(`https://backend.joyiapetroleum.com/joiya/idFillingStation/data/${params.id}`)
        res = await res.json();
...
/joiya/idFillingStation/data/${params.id}`, {
        ...
        res = await res.json();
        console.log(res);
        navigate('/id-filling-station/get-all-data');
    ...
    return (
        <>
            <div className="form-container card border-0 shadow rounded-3 my-5 p-5">
                <h1 className="mb-5">ID Filling Station</h1>
                <hr />
                <form>
                    {/* sup 1 */}
...
export default EditIDFillingStation